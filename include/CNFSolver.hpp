...
class CNFSolver
{
 ...
 private:
   ...
   bool findValidLineOrToken(const bool & checkFirstPos = false)
   {
      bool res = false;
      if (_posInTokenStack > 0)
      {
         size_t endI = 0;
         --endI;
         for (size_t i = _posInTokenStack - 1; i != endI && !res; --i)
         {
            _tokenStack[i + 1].clauseState.posInVec = 0;
            BranchToken & token = _tokenStack[i];
            size_t startI = token.clauseState.posInVec + ((checkFirstPos) ? 0 : 1);
            for (size_t j = startI; j < LitVector::size(); ++j)
            {
               if (token.litState.get(j))
               {
                  ...
                  res = true;
                  if (i < _lowestBackstepLvl || _posInTokenStack > _highestLvl)
                  {
                     std::cout << "Setting back to Stack(" << _posInTokenStack << "), Line(" << j << ")" << std::endl;
                     std::cout << "Conflics per sec: " << (_zeroCounter / std::max(1.0, std::floor((double) (clock() - _startTime) / CLOCKS_PER_SEC)))
                               << std::endl << "lowest backstep: " << _lowestBackstepLvl << " highest: " << _highestLvl << std::endl;
                  }
